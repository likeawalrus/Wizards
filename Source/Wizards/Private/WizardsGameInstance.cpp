...
bool UWizardsGameInstance::myJoinSession(TSharedPtr<const FUniqueNetId> UserId, FName SessionName, const FOnlineSessionSearchResult& SearchResult)
{
	// Return bool
	...
	if (SessionSearch->SearchResults.Num() > 0)
	{
		...
		for (int32 i = 0; i < SessionSearch->SearchResults.Num(); i++)
		{
			...
			if (SessionSearch->SearchResults[i].Session.OwningUserId != Player->GetPreferredUniqueNetId())
			{
				...
				myJoinSession(Player->GetPreferredUniqueNetId(), GameSessionName, SearchResult);
				break;
			}
		}
	}
