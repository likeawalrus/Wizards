...
void AWizardsProjectile::OnHit(class UPrimitiveComponent* HitComp, AActor* OtherActor, UPrimitiveComponent* OtherComp, FVector NormalImpulse, const FHitResult& Hit)
{
	...
	if (shouldBounce == false && (OtherActor != this) && (OtherActor != owningWizard) && explodeDeath) {
	...
	else if (shouldBounce == false) {
		Destroy();
	}
}

void AWizardsProjectile::SpellCreation(FtheSpell* theSpell, UParticleSystem* projPart, UParticleSystem* blastPart, TSubclassOf<class AWizardsBlast> myBlastClass, AWizardsCharacter* theWiz) {
	if (theSpell != NULL) {
		if (!projPart->GetWorld()) {
			MyParticleSystem->SetTemplate(projPart);
		}
		spellDamage = theSpell->spellDamage;
		spellEffect = theSpell->spellEffect;
		MyParticleSystem->AttachToComponent(RootComponent, FAttachmentTransformRules::KeepRelativeTransform);
